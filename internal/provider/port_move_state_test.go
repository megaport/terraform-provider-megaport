...
func TestMoveState_Port_V1ToV2(t *testing.T) {
	...
	assert.Equal(t, "staging", stringAttrValue(t, tagElements["env"]))
	assert.Equal(t, "infra", stringAttrValue(t, tagElements["team"]))
}
...
func TestMoveState_LagPort_V1ToV2(t *testing.T) {
	...
	assert.Equal(t, "lag-1", stringAttrValue(t, uidElements[0]))
	assert.Equal(t, "lag-4", stringAttrValue(t, uidElements[3]))
	...
	assert.Equal(t, "prod", stringAttrValue(t, tagElements["env"]))
}
