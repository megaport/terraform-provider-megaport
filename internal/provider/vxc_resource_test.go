...
// TestAccMegaportVXC_TransitInternetVLANChangeRejected asserts that ESD-1101
// Fix 3 (plan-time rejection) fires: changing ordered_vlan on a VXC whose
// state shows a TRANSIT CSP connection must produce a clean diagnostic at
// plan time rather than reaching the API.
func TestAccMegaportVXC_TransitInternetVLANChangeRejected(t *testing.T) {
	t.Parallel()
	defer acquireAccTestSlot(t)()
	...
	cfg := func(aEndVLAN int) string {
		...
		`, mcrLocationID, transitLocs[0], mcrName, vxcName, aEndVLAN)
	}

	resource.Test(t, resource.TestCase{
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			// Create with ordered_vlan = 0 (auto-assign).
			{
				Config: cfg(0),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("megaport_vxc.transit_vxc", "product_uid"),
			...
			// Attempt to change a_end.ordered_vlan — must fail at plan time.
			{
				Config:      cfg(123),
				ExpectError: regexp.MustCompile(`VLAN change not supported on Transit/AWS VXC`),
				PlanOnly:    true,
			},
		},
	})
