...
// is in state. An unset cost_centre reads back empty, so presence is the check
// here, not a non-empty value. TestCheckResourceAttrSet rejects both.
...
// lifecycle block in the configuration. cost_centre is left unset throughout,
// so it is the computed attribute the framework marks unknown.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// them here would fail the apply with an inconsistent result.
			{
				...
				ConfigPlanChecks: resource.ConfigPlanChecks{
					PreApply: []plancheck.PlanCheck{
						plancheck.ExpectUnknownValue("megaport_mve.mve", tfjsonpath.New("cost_centre")),
					},
				},
				Check: resource.ComposeAggregateTestCheckFunc(
