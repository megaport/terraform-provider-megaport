	...
	resp.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"wait_time": schema.Int64Attribute{
				Description: "Maximum time in minutes to wait for resources to finish provisioning during create and update operations, for a VXC to decommission during destroy, and for a VLAN that a destroyed VXC held to free before a VXC create or update reuses it, before timing out. Defaults to 10, minimum 1. Increase this if you provision resources that take longer than 10 minutes to become live, such as MVEs or VXCs to cloud providers.",
				Optional:    true,
...
func saveCreatedUID(ctx context.Context, resp *resource.CreateResponse, kind, name, uid string, waitErr error) bool {
	...
	if waitErr != nil {
		resp.Diagnostics.AddError(
			kind+" ordered but not ready",
			fmt.Sprintf("%s %s (%s) was ordered successfully but did not reach a ready state: %s. Its UID has been saved to state.", kind, name, uid, waitErr),
		)
		return false
	}
	return true
