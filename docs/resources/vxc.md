...
- `a_end_partner_config` (Attributes) The partner configuration of the A-End order configuration. Contains CSP and/or BGP Configuration settings. Only the "vrouter", "transit", and "a-end" partner configurations can be changed after the VXC is created. A change to a cloud partner configuration fails at plan time and leaves the VXC alone, because the provider does not send one on update. Imported VXCs do not have this field populated by the API. Adding the block after an import records it in Terraform state and warns you, because the API is never told about it. (see [below for nested schema](#nestedatt--a_end_partner_config))
- `b_end_partner_config` (Attributes) The partner configuration of the B-End order configuration. Contains CSP and/or BGP Configuration settings. Only the "vrouter" and "transit" partner configurations can be changed after the VXC is created. A change to a cloud partner configuration fails at plan time and leaves the VXC alone, because the provider does not send one on update. Imported VXCs do not have this field populated by the API. Adding the block after an import records it in Terraform state and warns you, because the API is never told about it. (see [below for nested schema](#nestedatt--b_end_partner_config))
